...
class BookCard extends StatelessWidget {
  ...
  BookCard(
      {@required this.book, this.info, @required this.category, this.color});
  ...
  void _settingModalBottomSheet(context) {
    category == 'eBook'
        ? Navigator.pushNamed(context, UIData.paymentRoute)
        : showModalBottomSheet(
            context: context,
            builder: (BuildContext bc) {
              return Container(
                child:
                    ShopBottomSheet(color: Colors.deepOrange[200], item: book),
                color: Colors.grey,
